...
#篩選出107/104大於1的職業別名稱之位置(因為107>104的有很多,所以篩選出為False的和NA值)
salaryAll$大學薪資比值>1
#篩選出職業別名稱(接著用負號的方式扣掉為false的和NA值的職業別名稱)
salaryAll$大職業別[-c(211,91,121,42,112,33,63,133,14,84,134,15,35,
                  105,115,135,16,56,126,136,7,17,77,28,98,118,19,
...
View(TOP10ratio)
#並用文字說明其結果
#根據中華民國統計資訊網顯示,103年第四季是4.38%,但到了104年第三季
#突然跌至-0.28%,是因為國際需求低靡,
...
#提高超過5%的職業別
#因為有空值所以要去除空值才會是應留的職業別,所以使用&連接符號將其兩個條件連接
...
#哪些行業男生薪資比女生薪資多,透過女/男<100可判出男生薪資大於女生的向量位置
salary104$`大學-女/男`<100
#再由位置篩選出職業別名稱,因為true較多,所以選出false和NA值
#再用負號做取出男生薪資大於女生薪資的職業別名稱
salary104$大職業別[-c(21,91,42,112,33,63,133,14,84,104,124,35,105,115,
                  135,56,126,7,77,97,28,98,118,19,49,119,40,70,140)]
...
#哪些行業女生薪資比男生薪資多,上面已經排序過,查表由位置篩選出職業別名稱
salary104$大職業別[c(112,113,114,115,116)]
...
#哪些行業男生薪資比女生薪資多,透過女/男<100可判出男生薪資大於女生的向量位置
salary107$`大學-女/男`<100
#再由位置篩選出職業別名稱,因為true較多,所以選出false的和NA值再用負號做取出男生薪資大於女生薪資的職業別名稱
...
#那些行業女生薪資比男生薪資多,上面已經排序過,查表由位置篩選出職業別名稱
#由於最多只有與男生薪資相,沒有大於男生薪資的
#此處將呈現與男生相等的職業別為何
...
salary107$大職業別[1:8]
#呈現前十名的資料(2分)
#由於女生薪資比男生薪資多的資料只有八筆,所以此處只呈現八筆資料
...
salary107$`研究所-薪資`<-gsub("…","",salary107$`研究所-薪資`)
#確認資料型別
is.numeric(salary107$`大學-薪資`)
is.numeric(salary107$`研究所-薪資`)
#轉換資料型別
